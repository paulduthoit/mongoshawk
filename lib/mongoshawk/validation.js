...
Validation.length = function(value, min, max) {
    ...
    // Check if match
    if(typeof(max) !== "undefined" && (value.length < min || value.length > max)) {
        return new ValidationException("Must contain between " + min + " and " + max + " character(s)");
    } else if(value.length < min) {
        return new ValidationException("Must contain minimum " + max + " character(s)");
...
Validation.range = function(value, min, max) {
    ...
    if(value < min) {
        ...
        return new ValidationException("Must be lower or equal to " + max);
    }
