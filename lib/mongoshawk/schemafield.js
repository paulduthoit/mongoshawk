...
function SchemaField(path, type, options) {
    ...
    if(typeof(path) !== "string")
        throw "path have to be a string";
    if(typeof(type) === "undefined")
        throw "type have to be defined";
    ...
    if(type instanceof Array) {
        this.array  = true;
        type = typeof(type[0]) !== "undefined" ? type[0] : Schema.Types.Mixed;
    }
 ...
 * @api public
 */
SchemaField.prototype.validate = function(value, action, callback) {
        ...
        function(asyncCallback) {
            ...
            if(self.array) {
                ...
                if(self.subSchema) {
                    ...
                    value.forEach(function(valueItem, valueIndex) {
                        ...
                        // Get sub schema validation
                        self.subSchema.validate(valueItem, action, function(subSchemaErrors) {

                            // Push to errors
                            if(subSchemaErrors) {
                                subSchemaErrors.error.datas.forEach(function(err) {
                ...
                // If field is not a sub-schema
                    ...
                    value.forEach(function(valueItem, valueIndex) {
                        ...
                        if(self.validation instanceof ValidationSet) {
                            ...
                            self.validation.validate(valueItem, action, function(validationErrors) {
                    
                                // Push to errors
                                if(validationErrors) {
                                    validationErrors.error.datas.forEach(function(err) {
                                        err = err.substr(0, 1).toLowerCase() + err.substr(1);
            ...
            // Must not be an array
                ...
                if(self.subSchema) {
                    ...
                    // Get sub schema validation
                    self.subSchema.validate(value, action, function(subSchemaErrors) {
                ...
                // If field is not a sub-schema
                    ...
                    if(value !== null && !self.type.validate(value)) {
                        ...
                        // Push to errors
                        if(["a", "e", "i", "o", "u", "y"].indexOf(self.type.name.substr(0, 1).toLowerCase()) !== -1) {
                            errors.push({ "field" : self.path, "message" : "The " + self.path + " field must be an " + self.type.name });
                        } else {
                    ...
                    if(self.validation instanceof ValidationSet) {
                        ...
                        // Get field validation
                        self.validation.validate(value, action, function(validationErrors) {
